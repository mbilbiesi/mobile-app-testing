...
public class RestaurantListScreenSteps extends BaseSteps {
    ...
    public RestaurantListScreenSteps(AppiumDriver driver, String language) {
        super(driver);
...
//        verifyScreenElements();
    ...
    public void checkRecommendedBadge(boolean isRestaurantRecommended) {
        ...
        if (restaurantCount > 0) {
            ...
            soft.assertThat(restaurantCount > 0)
                    .as("Unable to verify that recommended badge is displayed since " +
                            "there are no restaurants matching the searck keyword").isTrue();
        }
        clearSearchCriteria();
    ...
    public void checkIfRestaurantsSortedByDistance(boolean restaurantsWithBadges) {
        Assumptions.assumeThat(restaurantsWithBadges)
                .as("Restaurants sorting won't be correct because some restaurants are either recommended or" +
                        "sponsored")
                .isFalse();
        int restaurantCount = getRestaurantsCount(true);
...
//    waitUntilRestaurantsAreLoaded();
    ...
    public void waitUntilCampaignRestaurantsAreLoaded() {
        wait.until(ExpectedConditions.visibilityOfAllElements(restaurantsListScreen.getRestaurantWidgets()));
    }
    ...
    public boolean scrollDownRestaurantsList() {
        ...
        if (restaurantsListScreen.getPromotedBadge().size() > 0 || restaurantsListScreen.getRecommendedBadge().size() > 0) {
            return true;
        } else {
            return false;
        }
...
//        navigateBack(1);
    ...
    public void verifyMaxCampaginsNumberInCarousel() {
        ...
        while (activeScreenCampagins > 4) {
            ...
            if (!lastVisibleCampaignBeforeSwipe.getText().equalsIgnoreCase(lastVisibleCampaignAfterSwipe.getText())) {
